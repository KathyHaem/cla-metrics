...
@torch.no_grad()
def cache_prefix(model, tokenizer, lang, glossary, batch_size, is_base, is_thinking):
    if is_base:
        prompt = "\n\n".join([f"{gloss['description']}\n{gloss['label']}" for gloss in glossary[lang]]) + "\n\n"
        suffix = "\n"
    else:
        conversation = [{"role": "system", "content": ""}]
        for gloss in glossary[lang]:
            conversation.append({
                "role": "user",
        ...
        chat_applied = tokenizer.apply_chat_template(conversation, tokenize=False, add_generation_prompt=not is_thinking, continue_final_message=is_thinking)
        prompt, suffix = chat_applied.split("<SPLIT>")
    ...
    return {
        "cache": broadcast_cache(outputs.past_key_values, batch_size),
        "suffix": suffix
    }

@torch.no_grad()
def encode_batch(model, tokenizer, kv_cache, cache_mask, sentences, suffix="\n"):
    out_dict = {}
    if not tokenizer.pad_token:
        tokenizer.pad_token = tokenizer.eos_token
    tokenizer.padding_side = 'right'

    prompts = [f"{sent}{suffix}" for sent in sentences]
...
@torch.no_grad()
def get_fewshot_embeds(model, tokenizer, dataloader, lang, flores_code, sent_rep, is_base, is_thinking):
    ...
    caching_outputs = cache_prefix(model, tokenizer, lang, glossary, 1, is_base, is_thinking)
    cached_kv_1, _ = caching_outputs["cache"]
    suffix = caching_outputs["suffix"]

    for batch in tqdm(dataloader, desc=f"Processing {lang} / {flores_code}"):
        out_of_mem = False
        try:
            batch_results = encode_batch(model, tokenizer, *broadcast_cache(cached_kv_1, len(batch["sentence_"+flores_code])), sentences=batch["sentence_"+flores_code], suffix=suffix)
        ...
        if out_of_mem:
            split_batch = [encode_batch(model, 
                                        ...
                                        *broadcast_cache(cached_kv_1, 1), 
                                        sentences=[batch["sentence_"+flores_code][i]], suffix=suffix) for i in range(len(batch["sentence_"+flores_code]))]
            batch_results = dict()
...
if __name__ == "__main__":
    print("Directly running save_embeds_fewshot.py is deprecated. Use save_embeds.py instead.")
    exit(1)
